...
// --- transient failures: 429 (edge rate limit) / 502 / 503 / 504 -----------
// The NGINX ingress rate-limits per IP and the services shed load via
// Resilience4j (circuit breaker / rate limiter / bulkhead → 503). Reads are
// safe to retry with backoff; writes are NOT retried here — a replayed POST
// could double-place an order.
const TRANSIENT_STATUSES = new Set([429, 502, 503, 504])
const MAX_TRANSIENT_RETRIES = 2

function transientRetryDelayMs(attempt: number, retryAfterHeader?: string) {
  const retryAfterSec = Number(retryAfterHeader)
  if (!Number.isNaN(retryAfterSec) && retryAfterSec > 0) {
    return Math.min(retryAfterSec * 1000, 5_000)
  }
  return 400 * 2 ** attempt // 400ms, 800ms
}
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // Retry idempotent reads on transient gateway/service errors.
    const method = (original?.method ?? 'get').toLowerCase()
    const isIdempotent = method === 'get' || method === 'head'
    if (original && isIdempotent && status && TRANSIENT_STATUSES.has(status)) {
      const attempt = original._transientRetries ?? 0
      if (attempt < MAX_TRANSIENT_RETRIES) {
        original._transientRetries = attempt + 1
        const retryAfter = error.response?.headers?.['retry-after'] as
          | string
          | undefined
        await sleep(transientRetryDelayMs(attempt, retryAfter))
        return api(original)
      }
    }
...
export function toApiError(err: unknown): ApiError {
  if (axios.isAxiosError(err)) {
    ...
    return {
      status,
      message: data?.message || (status ? fallback : err.message || fallback),
